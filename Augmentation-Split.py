# -*- coding: utf-8 -*-
"""
Created on Sun Nov 18 17:57:29 2018
...
image_list2 = []
image_list3 = []
for filename1 in glob.glob("/home/dida/Documents/multifocus_CNN/1/*.png"): 
    img1_list = cv2.imread(filename1,0)
    image_list1.append(img1_list)
for filename2 in glob.glob("/home/dida/Documents/multifocus_CNN/2/*.png"): 
    img2_list = cv2.imread(filename2,0)
    image_list2.append(img2_list)
for filename3 in glob.glob("/home/dida/Documents/multifocus_CNN/labels/*.png"): 
    bmap_list = cv2.imread(filename3,0)
    image_list3.append(bmap_list)
   
# Load an color image in grayscale
#img1= cv2.imread('x1.png',0)
#img2=cv2.imread('x2.png',0)
#bmap= cv2.imread('Tgrd.jpg',0)
    ...
    while counter1 < samples or counter2 < samples:
         ...
         if np.size(Icrp1,0)==sheight and np.size(Icrp1,1)==swidth and np.var(Icrp1) >= 49:
             if bmapcrp[np.size(bmapcrp,0)//2,np.size(bmapcrp,1)//2] == 0 and counter1 < samples:
                     #path1 = 'C:\\Users\\dida\\Desktop\\example\\class0\\'
                     #cv2.imwrite(os.path.join(path1 , "%d.png"%((k*100)+counter1+1)), Icrp)
                     y.append(0)   
                     X.extend(Icrp1)   
                     counter1=counter1+1
             if bmapcrp[np.size(bmapcrp,0)//2,np.size(bmapcrp,1)//2] == 255 and counter2 < samples: 
                     #path2 = 'C:\\Users\\dida\\Desktop\\example\\class1\\'
                     #cv2.imwrite(os.path.join(path2 , "%d.png"%((k*100)+counter2+1)), Icrp)
                     y.append(1)
                     X.extend(Icrp1)
                     counter2=counter2+1
X = np.asarray(X)
...
#X_train,X_validate,X_test = np.split(X,[14400 , 19200])
#y_train,y_validate,y_test = np.split(y,[14400 , 19200])
